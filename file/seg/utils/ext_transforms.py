...
class ExtResize_with_padding(object):
    ...
    def __call__(self, img, lbl):
        ...
        new_image = Image.new('RGB', self.size, color=0)
        new_mask = Image.new('L', self.size, color=0)

        dest_h,dest_w= self.size
        ratio = max(w/dest_w,h/dest_h) 
        dest_size = (int(h/ratio),int(w/ratio))              
        pad_h= (dest_h-dest_size[0])//2
        pad_w = (dest_w-dest_size[1])//2

        img_paste  = F.resize(img, dest_size, self.interpolation)
        mask_paste = F.resize(lbl, dest_size, Image.NEAREST)

        new_image.paste(img_paste, (pad_w, pad_h))
        new_mask.paste(mask_paste, (pad_w, pad_h))
...
class Extmultiscale(object):
    def __init__(self, angle=0, translate=[0, 0], scale=[1.0,1.0], shear=[0, 0], seg_fill=0):
        self.angle = angle
    ...
    def __call__(self, img, label):# 
        ...
        new_scale = random.uniform(self.scale[0],self.scale[1])
        new_scale = new_scale if new_scale<1 else 1
        
        if random.random()<0.5:
            return TF.affine(img, self.angle, self.translate, new_scale, self.shear, TF.InterpolationMode.BILINEAR, 0), TF.affine(label, self.angle, self.translate, new_scale, self.shear, TF.InterpolationMode.NEAREST, self.seg_fill) 
        else:
            return img,label
